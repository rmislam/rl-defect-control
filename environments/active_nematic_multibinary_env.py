...
ALLOW_MULTIPLE_DEFECTS = readParam('ALLOW_MULTIPLE_DEFECTS')
...
class ActiveNematicEnv(gym.Env):
        ...
        with open(src_path + '/cpp/input/activity.dat', 'w') as f:
            for j in range(J_obs):
                for i in range(I_obs):
                    activity = ALPHA if self._isPointInActivityPattern(i, j, pattern_vector) else 0
    ...
    def setGoal(self, goal=None):
        if goal is not None:
            ...
            self._goal_frac_i = 0.5
            self._goal_frac_j = np.round(0.1 * np.random.random() + 0.7, 2)  # 0.7 to 0.8
            #self._goal_frac_i = 390 / 420
            #self._goal_frac_j = np.round(0.1 * np.random.random() + 0.2, 2)  # 0.2 to 0.3
    ...
    def _readIsTruncAndSetNearestDefect(self):
        ...
        with open(src_path + '/cpp/output/defect_obs.dat', 'r') as f:
            ...
            if int(tokens[0]) == -1:  # no defect was detected in simulation
                ...
                if ALLOW_MULTIPLE_DEFECTS == "TRUE":
                    # find nearest +1/2 defect from tokens_list
                    ...
                    # truncate episode if more than one +1/2 defect was found
                    if len(tokens_list) > 1:
                        is_trunc = True
                        self._nearest_defect_i = None
                        self._nearest_defect_j = None
                    else:
                        self._nearest_defect_i = tokens_list[0][0]
                        self._nearest_defect_j = tokens_list[0][1]
