...
def computeMinMaxFreeEnergy():
    min_fe = 100
    max_fe = -100

    for i in range(NUM_FILES):
        df_free_energy = pd.read_csv(OUTPUT_FILE_PATH + 'free_energy_' + str(i * TIME_WRITE) + '.dat', sep=' ', header=None)
        free_energy = df_free_energy.iloc[:, 2]
        non_nan_free_energy = free_energy[~free_energy.isnull()]
        fe_max = non_nan_free_energy.max()
        fe_min = non_nan_free_energy.min()
    ...
    return min_fe, max_fe


min_fe, max_fe = computeMinMaxFreeEnergy()
...
fig_cbar = plt.figure(figsize=(1.5, 7))

# Create dedicated axes for colorbar only
cax_cbar = fig_cbar.add_axes([0.35, 0.05, 0.3, 0.9])
# [left, bottom, width, height] in figure fraction

#fig_cbar, ax_cbar = plt.subplots()
#fig_cbar.set_figheight(4)
#fig_cbar.set_figwidth(1)
#fig_cbar.subplots_adjust(left=0, bottom=0, right=1, top=1, wspace=None, hspace=None)

def log_exponent_formatter(x, pos):
    return rf'${int(np.log10(x))}$'

fe_log_norm = LogNorm(vmin=vmin, vmax=max_fe - min_fe + vmin)

# im1 = ax1.imshow(np.ones((J, I)), cmap=cmap_fe, origin="lower",
#                  norm=fe_log_norm)

# cax1 = inset_axes(ax1,
#                  width="5%",   # % of parent_bbox width
#                  height="20%", # % of parent_bbox height
#                  loc="lower left",
#                  bbox_to_anchor=(0, 0, 1, 1),  # shift right
#                  bbox_transform=ax1.transAxes,
#                  borderpad=2)

# ax1.set_aspect('equal', adjustable='box')
# cbar1 = fig1.colorbar(im1, cax=cax1, format=log_exponent_formatter)
# cbar1.ax.tick_params(labelsize=8)
# cbar1.ax.set_title(r'$\log_{10}[f \,]$', fontsize=8)

# im2 = ax2.imshow(np.ones((J, I)), cmap=cmap_fe, origin="lower",
#                  norm=fe_log_norm)
...
# cbar2 = fig2.colorbar(im2, cax=cax2, format=log_exponent_formatter)
# cbar2.ax.tick_params(labelsize=8)
# cbar2.ax.set_title(r'$\log_{10}[f \,]$', fontsize=8)

sm = ScalarMappable(norm=fe_log_norm, cmap=cmap_fe)
sm.set_array([])
cbar = fig_cbar.colorbar(sm, cax=cax_cbar, format=log_exponent_formatter)
#cbar.ax.set_title(r'$\log_{10}[f \,]$', fontsize=8)
cbar.minorticks_off()
cbar.ax.tick_params(labelsize=18)
cbar.set_label(r'$\log_{10}[f \;]$', rotation=90, labelpad=10, fontsize=22)
fig_cbar.savefig(dir_path + f'/output/free_energy_colorbar.pdf', bbox_inches='tight', dpi=600)
...
def updateOrientationFigure(i):
    ...
    free_energy -= min_fe
    free_energy += vmin
    free_energy[free_energy.isnull()] = vmin
    ax2.imshow(free_energy.to_numpy().reshape(J, I), cmap=cmap_fe, origin='lower', interpolation=interp_method, norm=LogNorm(vmin=vmin, vmax=max_fe - min_fe + vmin))
    ...
    if timestep_idx in stills_index_list:
        ...
        fig2.savefig(dir_path + f'/output/stills/orientation_FE_{GEOM_NAME}_{sync_timestep}.pdf', dpi=600)
...
def updateVelocityFigure(i):
    ...
    free_energy += vmin
    free_energy[free_energy.isnull()] = vmin
    ax1.imshow(free_energy.to_numpy().reshape(J, I), cmap=cmap_fe, origin='lower', interpolation=interp_method, norm=LogNorm(vmin=vmin, vmax=max_fe - min_fe + vmin))
    ...
    if timestep_idx in stills_index_list:
        sync_timestep = int(timestep_idx / TIME_SYNC)
        fig1.savefig(dir_path + f'/output/stills/velocity_FE_{GEOM_NAME}_{sync_timestep}.pdf', dpi=600)
...
orientation_animation_fig.save(dir_path + '/output/orientation_FE.mp4', writer='ffmpeg', dpi=600, savefig_kwargs=dict(facecolor='xkcd:white'))
...
velocity_animation_fig.save(dir_path + '/output/velocity_FE.mp4', writer='ffmpeg', dpi=600, savefig_kwargs=dict(facecolor='xkcd:white'))