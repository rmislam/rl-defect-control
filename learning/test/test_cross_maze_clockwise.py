...
if __name__ == "__main__":
    ...
    model_down = PPO.load(dir_path + '/../saved_models/ppo-active-nematic-multibinary-CROSS_JUNCTION-down/ppo-active-nematic-multibinary-CROSS_JUNCTION_192000_steps.zip', env=env)
    ...
    while True:
        ...
        if num_steps == 7:
            ...
            env.setQuadrant('BL')  # be sure to set quadrant before doing anything else
            env.setRotation(2)
            goal_frac_i = 0.5
            goal_frac_j = 90 / 420
            env.setGoal(goal=[goal_frac_i, goal_frac_j])
